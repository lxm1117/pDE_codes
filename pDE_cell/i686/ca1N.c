	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 ca1N /home/ximing/Documents/lab_stuff/pDE_cell/i686/ca1N.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "ca1c.mod Conductances for CA1 cells";
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 52 in file ca1N.mod:\n        SOLVE states METHOD cnexp \n"); nrn_complain(_p); abort_run(error);}
...
static _initlists() {
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(nna) - _p;  _dlist1[0] = &(Dnna) - _p;
 _slist1[1] = &(nni) - _p;  _dlist1[1] = &(Dnni) - _p;
_first = 0;
}
